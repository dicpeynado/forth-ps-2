...
#ifndef ENABLE_BITLUNI_SUPPORT
#define OPTIONAL_BITLUNI_SUPPORT
#else
  #undef a0
  #undef a1
  #undef a2
...
#define OPTIONAL_BITLUNI_SUPPORT \
  X("VGA.init320x240", VGA_INIT320X240, VGA.init(VGA.MODE320x240,rPin0,rPin1,gPin0,gPin1,bPin0,bPin1,hsyncPin,vsyncPin)) \
  X("VGA.init320x480", VGA_INIT320X480, VGA.init(VGA.MODE320x480,rPin0,rPin1,gPin0,gPin1,bPin0,bPin1,hsyncPin,vsyncPin)) \
  X("VGA.init360x400", VGA_INIT360X400, VGA.init(VGA.MODE360x400,rPin0,rPin1,gPin0,gPin1,bPin0,bPin1,hsyncPin,vsyncPin)) \
  X("VGA.init400x300", VGA_INIT400X300, VGA.init(VGA.MODE400x300,rPin0,rPin1,gPin0,gPin1,bPin0,bPin1,hsyncPin,vsyncPin)) \
  X("VGA.init720x350", VGA_INIT720x350, VGA.init(VGA.MODE720x350,rPin0,rPin1,gPin0,gPin1,bPin0,bPin1,hsyncPin,vsyncPin)) \
  X("VGA.init800x600", VGA_INIT800X600, VGA.init(VGA.MODE800x600,rPin0,rPin1,gPin0,gPin1,bPin0,bPin1,hsyncPin,vsyncPin)) \
  X("VGA.init640x400", VGA_INIT640X400, VGA.init(VGA.MODE640x400,rPin0,rPin1,gPin0,gPin1,bPin0,bPin1,hsyncPin,vsyncPin)) \
  X("VGA.setFont6x8", VGA_SETFONT6X8, VGA.setFont(Font6x8); VGA.autoScroll=true;) \
  X("VGA.setFont8x8", VGA_SETFONT8X8, VGA.setFont(Font8x8); VGA.autoScroll=true;) \
  X("VGA.setTextColor", VGA_SETTEXTCOLOR, VGA.setTextColor((VGA3BitI::Color) n1, (VGA3BitI::Color) n0); DROPn(2)) \
  X("VGA.setCursor", VGA_SETCURSOR, VGA.setCursor(n1,n0); DROPn(2)) \
  X("VGA.RGB", VGA_RGB, PUSH VGA.RGB(n3, n2, n1); NIPn(3)) \
  X("VGA.print", VGA_PRINT, VGA.print(c0); DROP) \
  X("VGA.printc", VGA_PRINTC, VGA.print((char) n0); DROP) \
  X("VGA.println", VGA_PRINTLN, VGA.println(c0); DROP) \
  X("VGA.clear", VGA_CLEAR, VGA.clear((VGA3BitI::Color) n0); DROP) \
  X("VGA.rect", VGA_RECT, VGA.rect(n4, n3, n2, n1, (VGA3BitI::Color) n0); DROPn(5)) \
  X("VGA.fillRect", VGA_FILLRECT, VGA.fillRect(n4, n3, n2, n1, (VGA3BitI::Color) n0); DROPn(5))
...
"   15 for dup 63 type [char] | emit 15 r@ - . cr 64 + next drop ;\n"
"internals definitions\n"
": @line ( n -- ) 64 * scr @ block + ;\n"
